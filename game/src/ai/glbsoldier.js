...
export async function loadGlbSoldier(url, { height = 1.8, name = 'glb_soldier' } = {}) {
  ...
  const yaw = Math.atan2(fwd.x, fwd.z);            // rotate by -yaw so fwd lands on +Z
  ...
  g.updateMatrixWorld(true);
  if (lHip && rHip && wp(lHip).x < wp(rHip).x) console.warn(`[glbsoldier] ${name}: left hip is on -X after facing; mirrored skeleton?`);

  const joints = {};
  for (const [j, names] of Object.entries(BONE_MAP)) { const b = find(names); if (b) joints[j] = b; else console.warn(`[glbsoldier] ${name}: no bone for ${j} (${names.join('/')})`); }
  const hand = find(['RightHand']);
  ...
  console.log(`[glbsoldier] ${name}: ${tris} tris, ${Object.keys(bones).length} bones, yaw ${(yaw * 180 / Math.PI).toFixed(0)} deg, scale ${k.toFixed(3)}, joints ${Object.keys(joints).join(',')}`);
  return g;
}
