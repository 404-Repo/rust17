...
const CFG = { kind: 'tan', livery: 0xb0a07c, bleach: 0.22, plates: [{ on: 'door', w: 0.4, h: 0.3 }], band: { h: 0.4, color: 0x9c4a3c }, doorOpen: 30, rustRibs: true };
  ...
  function sideWall(sz) {
    ...
    for (const [y0, y1, mat] of bands) {
      const geo = corrExtrude(panelL, y1 - y0, PITCH, RIBD, SHEET, mod);
      mesh(g, geo, mat, 0, (y0 + y1) / 2, zBack, 0, sz > 0 ? 0 : PI, 0);
    }
    ...
    for (let i = 0; i < 7; i++) {
      ...
      streak(g, sz > 0 ? 'pz' : 'nz', x, H - 0.09, sz * (W / 2 - 0.01), 0.18 + 0.12 * (i % 3), 0.05);
    }
