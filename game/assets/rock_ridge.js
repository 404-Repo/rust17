...
export default function (THREE) {
  ...
  // round 18c: two sided, the bands read as open shells from below with front faces only
const mat = (hex, name, r = 0.92, mt = 0.0, flat = false) => { const m = new THREE.MeshStandardMaterial({ color: hex, roughness: r, metalness: mt, flatShading: flat, side: THREE.DoubleSide }); m.name = name; return m; };
  ...
  const skirt = (pts, cx, cz, out, hIn) => {
    ...
    const rows = [[0.0, hIn, -(0.45)], [0.42, hIn * 0.55, 0], [0.75, hIn * 0.18, 0], [1.0, 0, 0]];
